...
void render()
{
    ...
    float move = 0.25f;
    float scale = fabs(sin(glfwGetTime() / 3.0f)) / 2.0f + 0.5f;
    float circleX = sin(glfwGetTime()*3);
    float circleY = cos(glfwGetTime()*3);

    float rainbow[3];
    rainbow[0] = sin(glfwGetTime()*3.0) / 2.0f + 0.5f;
    rainbow[1] = cos(glfwGetTime()*3.0) / 2.0f + 0.5f;
    rainbow[2] = 1.0f;  

    float theta = glfwGetTime();
    float rotation[4] = {cos(theta), -sin(theta), sin(theta), cos(theta)};
    ...
    glUniform1f(glGetUniformLocation(shader, "move"), move);
    glUniform1f(glGetUniformLocation(shader, "scale"), scale);
    glUniform1f(glGetUniformLocation(shader, "circleX"), circleX);
    glUniform1f(glGetUniformLocation(shader, "circleY"), circleY);
    glUniform3f(glGetUniformLocation(shader, "rainbow"), rainbow[0], rainbow[1], rainbow[2]);
    glUniform4f(glGetUniformLocation(shader, "rotation"), rotation[0], rotation[1], rotation[2], rotation[3]);
