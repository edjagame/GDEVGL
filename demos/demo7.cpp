...
bool setupFbo()
{
    ...
    fboShader = gdevLoadShader("postProcessing.vs", "postProcessing.fs");
    if (! fboShader)
        return false;
...
bool setup()
{
    ...
    shader = gdevLoadShader("demos/demo5.vs", "demos/demo5.fs");
    if (! shader)
        return false;

    // load our texture
    texture = gdevLoadTexture("demos/demo5.png", GL_REPEAT, true, true);
    if (! texture)
        return false;
...
void render()
{
    ...
    // Use current width and height for aspect ratio
